...
**We already have an intuitive and accessible interface for robot learning: natural language**. We explore an approach for training robotic skills solely through natural language: 

* Collect robotic data through natural language supervision
* A VLA model learns visuomotor policies directly from language supervision
*****
...
<Figure
    caption="Experimental results on 18 robot manipulation tasks">
...
* CLIP-RT outperforms OpenVLA in average success rates by **3% on common tasks and 24% on novel manipulation tasks**.
* **Natural language supervision** significantly increases performance (vs CLIP-RT-Action)
* **STD** boosts the overall performance (vs CLIP-RT-Passive)


### Why does CLIP-RT perform well on Novel tasks?  

<Figure
    caption="Comparison between multi-task and single-task policies">
    <Image source={exp_results2} altText="Comparison between multi-task and single-task policies." style={{ width: '70%', height: 'auto' }} />
</Figure>

CLIP-RT benefits more on multi-task policies (+11.1%) compared with OpenVLA (+3.3%). It indicates that CLIP-RT learns more generalizable and transferable representations across diverse robotic tasks. 
